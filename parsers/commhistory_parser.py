...
def _handle_messages(cursor):
    ret = []
    successful = 0
    failed = 0
    for row in cursor.execute(
            "SELECT id, type, startTime, endTime, direction, isDraft, isRead, remoteUid, freeText, mmsId, hasMessageParts, headers FROM Events WHERE type IN (2, 6)"):
        msg, parsed = handle_message(row)
        if parsed is not None:
            #     warnings.warn("An error occurred. Skipping entry with id = " + str(row["id"]))
            #     continue
            for key in parsed:
                msg[key] = parsed[key]
            #print("Added entry = " + str(parsed))
            successful += 1
            ret.append(msg)
        else:
            failed += 1
            #print("Skipped entry")
    print("Successful: " + str(successful) + " failed: " + str(failed))
    return ret


def handle_message(row):
    if row['type'] == 2:
        msg, parsed = parse_sms(row)
    else:
        msg, parsed = parse_mms(row)
    return msg, parsed
...
def parse_mms(row):
    ...
    parsed = dict(
        ...
        sub_cs="null",
        st="null",
        creator="org.freedesktop.Telepathy.Client.CommHistory",
        tr_id="null",
        sub_id="1",
        resp_st="null",
        m_id=row["mmsId"],
    ...
    try:
        ...
        warnings.warn("No SMIL file found")
        return None, None
    return msg, parsed
...
def parse_sms(row):
    ...
    if parsed["address"] == "" or parsed["address"] is None:
        if parsed["type"] == MessageBox.DRAFT:
            parsed["address"] = "null"
        else:
            warnings.warn("Setting recipient to " + RECIPIENT + " for message with id = " + str(row["id"]))
...
def _handle_calls(cursor):
    ret = []
    for row in cursor.execute(
            "SELECT id, type, startTime, endTime, direction, isMissedCall, remoteUid FROM Events WHERE type = 3"):
        call = Call()
        ...
        parsed = dict(
            ...
            presentation=NumberPresentation.RESTRICTED if ((not row['remoteUid']) or (
                not any(char.isdigit() for char in row['remoteUid']))) else NumberPresentation.ALLOWED
        )
...
class CommhistoryParser():
    ...
    def parse(srcfile, sms_list=None, call_list=None):
        ...
        try:
            ...
            cur = conn.cursor()
            if sms_list:
                sms_list.extend(_handle_messages(cur))
            if call_list:
